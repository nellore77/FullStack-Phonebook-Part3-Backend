  ...
  PhoneBook.findById(id)
    .then((person) => {
      if (person) {
        ...
        res.status(404).send('Requested person not present');
      }
    })
    /* .catch((error) => {
  ...
  PhoneBook.findByIdAndDelete(id)
    .then((deletedPerson) => {
      if (!deletedPerson) {
        ...
        return res.status(404).send('Requested person not present');
      }
